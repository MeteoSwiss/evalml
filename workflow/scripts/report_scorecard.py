...
# ── Open + select model and baseline, compute (model − baseline) / |baseline| in % ──
...
_metric_w_pt = (
    _t2.get_window_extent(_fig_tmp2.canvas.get_renderer()).width * 72 / _fig_tmp2.dpi
)
plt.close(_fig_tmp2)
...
if _init_raw is not None:
    _title_parts.append(f"init={'all' if _init_raw == -999 else f'{_init_raw:02d}Z'}")
...
for row_idx, (group, metric) in enumerate(rows):
    ...
    for sec_idx, _slice in enumerate(_slices):
        ...
        for lt_idx, d in enumerate(diff_filtered[name].sel({strat_dim: _slice}).values):
            x = x_off + lt_idx
            if np.isnan(d):
                ax.plot(
                    x,
...
hline_x_start = metric_frac - ((_metric_w_pt - hline["start_pad_pt"]) / 72) / ax_w_in
