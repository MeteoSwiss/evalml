...
rule run_verif_compute_statistics:
    localrule: True
    input:
        script="workflow/scripts/verif_compute_statistics.py",
        grib_output=rules.map_init_time_to_inference_group.output[0],
    output:
        OUT_ROOT / "data/runs/{run_id}/{init_time}/statistics.nc",
    log:
        OUT_ROOT / "logs/verif_compute_statistics/{run_id}-{init_time}.log",
    shell:
        """
        uv run {input.script} --grib_output_dir {input.grib_output} --output {output} > {log} 2>&1
        """


rule run_verif_aggregate_statistics:
    localrule: True
    input:
        statistics=lambda wc: expand(
            rules.run_verif_compute_statistics.output,
            init_time=[t.strftime("%Y%m%d%H%M") for t in REFTIMES],
            run_id=collect_all_runs(),
        ),
    output:
        OUT_ROOT / "data/runs/{run_id}/statistics_aggregated.nc",
    log:
        OUT_ROOT / "logs/verif_aggregate_statistics/{run_id}.log",
    run:
        import xarray as xr
        ...
        encoding = {"ref_time": {"units": "hours since 2020-01-01T12:00:00"}}
        ds.to_netcdf(output[0], mode="w", format="NETCDF4", encoding=encoding)
