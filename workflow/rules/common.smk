    ...
    for entry in cfg.get("regions", []):
        if isinstance(entry, str):
            if entry in PREDEFINED_REGIONS:
                logging.getLogger("snakemake").info("Region %s found among the predefined regions (rectangles in lon/lat).  The evaluation will happen on the bounding box %s", entry, PREDEFINED_REGIONS[entry])
                result.append(
                    {"type": "bbox", "name": entry, "bbox": PREDEFINED_REGIONS[entry]}
                )
            else:
                path = f"{root}/{entry}.shp"
                logging.getLogger("snakemake").info("Region %s not found in the predefined regions. Will try to locate file %s.", entry, path)
                result.append(
                    {"type": "shp", "name": entry, "path": path}
                )
